...
public class TemporaryLobCreator implements LobCreator {
	...
	protected static final Logger logger = LogFactory.getLogger(TemporaryLobCreator.class);
	...
	public void setBlobAsBinaryStream(
		...
		Blob blob = ps.getConnection().createBlob();
		try {
			SpringUtils.copy(binaryStream, blob.setBinaryStream(1));
		}
	...
	public void setClobAsAsciiStream(
		...
		Clob clob = ps.getConnection().createClob();
		try {
			SpringUtils.copy(asciiStream, clob.setAsciiStream(1));
		}
		catch (IOException ex) {
			throw new DataAccessResourceFailureException("Could not copy into LOB stream", ex);
		}
	...
	public void setClobAsCharacterStream(
		...
		Clob clob = ps.getConnection().createClob();
		try {
			SpringUtils.copy(characterStream, clob.setCharacterStream(1));
		}
		catch (IOException ex) {
			throw new DataAccessResourceFailureException("Could not copy into LOB stream", ex);
		}
