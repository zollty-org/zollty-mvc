...
public class ErrorHandler implements WebHandler {
	...
	/* (non-Javadoc)
	 * @see org.zollty.framework.mvc.handler.WebHandler#invoke(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, org.zollty.framework.mvc.handler.HandlerChain)
	 */
	@Override
	public View invoke(HttpServletRequest request,
	...
	public void render(HttpServletRequest request, HttpServletResponse response){
		try {
			...
			if(null!=v) v.render(request, response);
		} catch (Throwable e) {
	 ...
	 */
	public View doErrorPage(HttpServletRequest request,
		...
		// 记录错误信息
		if(log.isInfoEnabled()) log.error("ErrorMsg: "+msg+", StackTrace: "+MvcUtils.ExceptionUtil.getStackTraceStr(e));
	...
	public String defaultErrorPage() {
		...
		try {
			...
			.append("<h2>HTTP ERROR ").append( status==-1?"":Integer.toString(status) ).append("</h2><div>")
			.append(MvcUtils.StringUtil.simpleHtmlEscape(
					URLDecoder.decode( MvcUtils.StringUtil.isNotBlank(msg)?msg:(MvcUtils.ExceptionUtil.getStackTraceStr(e)),"UTF-8")
					))
...
//	public String defaultAlertErrorPage() {
//		StringBuilder ret = new StringBuilder();
//		try {
//			ret.append("<!DOCTYPE html><html><head><title>Unexpect Error Occured</title><script>function doAlert(){alert('")
//			.append("HTTP ERROR ").append(status==-1?"":Integer.toString(status))
//			.append("');}</script></head><body onload=\"doAlert()\">")
//			.append("<h2>HTTP ERROR ").append( status==-1?"":Integer.toString(status) ).append("</h2><div>")
//			.append(MvcUtils.escapeXML(
//					URLDecoder.decode( MvcUtils.isNotBlank(msg)?msg:(MvcUtils.getStackTraceStr(e, null)),"UTF-8")
//					))
//			.append("</div><hr/><div style=\"font-style: italic; font-family: Baskerville, 'Group Old Style', Palatino, 'Book Antiqua', serif;\"><small>Created-By: ZolltyMVC Framework ").append(
//					MvcDateFormatUtils.defaultDateFormat.format(new Date())
//					).append("</small></div></body></html>");
//		} catch (UnsupportedEncodingException e) {
//			log.error(e);
//		}
