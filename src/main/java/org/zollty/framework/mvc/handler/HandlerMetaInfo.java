...
public abstract class HandlerMetaInfo {
    ...
    private View doBefore(HttpServletRequest request, HttpServletResponse response, MvcContext mvcContext){
        if (beforeInvoke != null && beforeInvoke.length > 0) {
            for (MvcBeforeBeanDefinition mbd : beforeInvoke) {
                ...
                try {
                    ...
                    return new ErrorHandler(null, "AOP Before Method invoke error: " + e.getMessage(),
                            HttpServletResponse.SC_INTERNAL_SERVER_ERROR).doErrorPage(request, response);
                }
                catch (InvocationTargetException e) {
                    return new ErrorHandler(e.getTargetException(), "AOP Before Method invoke error",
                            HttpServletResponse.SC_INTERNAL_SERVER_ERROR).doErrorPage(request, response);
                }
                catch (Throwable e) {
    ...
    private View doBeforeRender(HttpServletRequest request, HttpServletResponse response, MvcContext mvcContext){
        return null;
    }
    
    private View doAfterThrowing(HttpServletRequest request, HttpServletResponse response, MvcContext mvcContext){
        return null;
    }

    public final View invokeMethod(InvokeParamsAdapter paramsHolder, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            return new ErrorHandler(e, "handler invoke error", HttpServletResponse.SC_INTERNAL_SERVER_ERROR)
